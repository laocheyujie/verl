...
def calculate_score(prediction, reference):
    ...
    return len(common) / len(reference) if reference else 0.0

def compute_score(data_source, solution_str, ground_truth, extra_info=None):
    assert data_source == "chinese_general_mcq", "data_source must be chinese_general_mcq"
    prediction = extract_answer(solution_str)
    score = calculate_score(prediction, ground_truth)
    print(f"{solution_str[:10]}... prediction: {prediction}, ground_truth: {ground_truth}, score: {score}")
    return score


if __name__ == "__main__":
    print(compute_score("chinese_general_mcq", "答案: A", "A"))
    print(compute_score("chinese_general_mcq", "答案：ABC", "ABC"))
    print(compute_score("chinese_general_mcq", "答案：BC", "ABC"))
    print(compute_score("chinese_general_mcq", "答案：BCD", "ABC"))